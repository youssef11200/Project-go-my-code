...
const PlaceOrderScreen = ({ history }) => {
  ...
  const creatOrder = useSelector((state) => state.orderCreate);
  const { order, succes, error } = creatOrder;
  useEffect(() => {
    if (succes) {
      history.push(`/order/${order._id}`);
      dispatch({ type: ORDER_CREATE_RESET });
    }
  }, [history, dispatch, succes, order]);
  console.log(cart)
  const placeOrderHandler = () => {
    ...
    dispatch(createOrder({
      ...
      ordershippingPrice: cart.shippingPrice,
